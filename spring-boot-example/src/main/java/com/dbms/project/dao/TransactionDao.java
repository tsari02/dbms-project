...
public class TransactionDao {
    ...
    public int insertTransaction(Transaction transaction) {
        final String sql = "INSERT INTO transaction(bankBranch, accountNumber, amount, mode, dateOfTransaction, bankName) VALUES(?, ?, ?, ?, ?, ?)";
        ...
        jdbcTemplate.update(connection -> {
            ...
            ps.setString(4, transaction.getMode());
            ps.setDate(5, transaction.getDateOfTransaction());
    ...
    public int updateTransaction(int id, Transaction transaction) {
        final String sql = "UPDATE transaction SET bankBranch = ?, accountNumber = ?, amount = ?, mode = ?, DateOfTransaction = ?, BankName = ? WHERE id = ?";
        return jdbcTemplate.update(sql, transaction.getBankBranch(), transaction.getAccountNumber(), transaction.getAmount(), transaction.getMode(), transaction.getDateOfTransaction(), transaction.getBankName(), id);
    }
}
